...
def test_stale_connector_instance_cannot_reverse_terminal_status(
    ...
    class CountingConnector(FakeReadConnector):
        ...
        def synchronize(
            ...
            return super().synchronize(read_scope)

    revoking_instance = ConnectorService(path=state_path)
    revoking_instance.connect(
        CountingConnector(), read_scope=("events:read",)
    )
    stale_adapter = CountingConnector()
