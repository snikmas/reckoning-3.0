...
`reckoning-telegram` for the Telegram webhook, and `reckoning-ops` for
operations.

## Run the Telegram webhook
...
IDs even when Telegram supplies the correct webhook secret. The channel accepts
ordinary messages, `/status`, and `/confirm <confirmation-id>`. It does not
provide Control or operational commands.
